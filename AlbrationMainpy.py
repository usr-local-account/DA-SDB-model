...
from AlbrationNet import TransferNet, AblationNet
...
def Main_train_Transfer(args):
    train_source_dataset = CustomDataset(csv_file=args.train_source_root, input_dim=args.input_features, use_slope=args.use_slope)
    ...
    train_target_dataset = CustomDataset(csv_file=args.train_target_root, input_dim=args.input_features, use_slope=args.use_slope)
    ...
    model = AblationNet(input_dims=args.input_features-1+args.slope_dims, 
                        hidden_dims=args.hidden_dim, dropout_prob=args.dropout,
    ...
    for epoch in range(args.domain_epochs):   
        ...
        if avg_mse < best_loss:
            best_loss = avg_mse
            torch.save(model.state_dict(), 'best_AblationNet.pth')
            stop = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', type=str, default='AblationRrsBlock',help='Choose the backbone networks')
    parser.add_argument('-hidden_dim', default=64, type=int,help='Dimension of hidden on regressior')
    parser.add_argument('-use_slope', default=True, help='the bool parms for slope.')
    parser.add_argument('-slope_dims', default=6, type=int, help='the dims for slope.')
    parser.add_argument('-batch_size', default=1024, type=int, metavar='N',help='mini-batch size (default: 64)') 
    parser.add_argument('--lr', default=0.004596632061999526, type=float, metavar='LR', help='initial learning rate', dest='lr')
    parser.add_argument('--dropout', default=0.1655990900531626, type=float, metavar='LR')
